...
class DecodeModel(pl.LightningModule):
    ...
    def beam_search(
        ...
        batch_indices = torch.arange(0, batch_size // 2, dtype=torch.long, device=self.device)
        best_indices = (
...
# From https://github.com/SJTU-DeepVisionLab/PosFormer/blob/main/Pos_Former/utils/generation_utils.py
class PosDecodeModel(pl.LightningModule):
    ...
    def _beam_search(
        ...
        while cur_len < max_len and not beam_scorer.is_done():
            ...
            next_indices = next_tokens // 6
            next_tokens = next_tokens % 6
