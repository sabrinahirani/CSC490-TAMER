...
from tamer.utils.generation_utils import DecodeModel, PosDecodeModel
...
# From https://github.com/SJTU-DeepVisionLab/PosFormer/blob/main/Pos_Former/model/decoder.py
class PosDecoder(PosDecodeModel):
    ...
    def transform(
            self, src: List[FloatTensor], src_mask: List[LongTensor],
            input_ids: LongTensor
    ) -> FloatTensor:
        assert len(src) == 1 and len(src_mask) == 1
        out_pos, _ = self(src[0], src_mask[0], input_ids,
